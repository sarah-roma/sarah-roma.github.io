...
exports.stripQuotes = exports.getTagName = exports.convertObjectLiteralToJs = exports.assertIsIdentifier = exports.assertIsObjectLiteralExpression = exports.assertIsArrayLiteralExpression = exports.assertIsStringLiteral = exports.assertIsPropertyAssignment = exports.parseFunctionArgument = exports.isOneOf = exports.parsePropertyOfType = exports.bfsFindNode = exports.getDefaultExport = void 0;
...
function parsePropertyOfType({ objectLiteralNode, propertyName, predicate, parserContext, required = false, errorMessage, }) {
    ...
    if (!node) {
        if (!required) {
            ...
            throw new parser_1.ParserError(`Expected property '${propertyName}' to be present`, {
                sourceFile,
    ...
    if (typescript_1.default.isShorthandPropertyAssignment(node)) {
        ...
        if (!symbol || !symbol.valueDeclaration || !typescript_1.default.isVariableDeclaration(symbol.valueDeclaration)) {
            throw new parser_1.ParserError('Expected shorthand property to be declared in the same file', {
                sourceFile,
                node,
            });
        }
    ...
    if (!initializer) {
        throw new parser_1.ParserError(`Expected property ${propertyName} to be a property assignment`, {
            sourceFile,
            node,
        });
    }
    ...
    if (!predicate(initializer)) {
        throw new parser_1.ParserError(errorMessage ??
...
function parseFunctionArgument(fn, parserContext, index, predicate, required = false, errorMessage) {
    ...
    if (fn.arguments.length <= index) {
        throw new parser_1.ParserError(errorMessage ?? `Expected function to have at least ${index + 1} arguments`, {
            sourceFile,
            node: fn,
        });
    }
    ...
    if (!arg || !predicate(arg)) {
        throw new parser_1.ParserError(errorMessage ?? `Unexpected shape of argument ${index}`, {
            sourceFile,
...
function assertIsPropertyAssignment(node, sourceFile) {
    if (!typescript_1.default.isPropertyAssignment(node)) {
        throw new parser_1.ParserError(`Expected a property assignment, got ${typescript_1.default.SyntaxKind[node.kind]}`, {
            node,
            sourceFile,
        });
    }
...
function assertIsStringLiteral(node, sourceFile, msg) {
    if (!typescript_1.default.isStringLiteral(node)) {
        throw new parser_1.ParserError(msg ?? `Expected a string literal, got ${typescript_1.default.SyntaxKind[node.kind]}`, {
            node,
            sourceFile,
        });
    }
...
function assertIsArrayLiteralExpression(node, sourceFile, msg) {
    if (!typescript_1.default.isArrayLiteralExpression(node)) {
        throw new parser_1.ParserError(msg ?? `Expected an array literal, got ${typescript_1.default.SyntaxKind[node.kind]}`, {
            node,
            sourceFile,
        });
    }
...
function assertIsObjectLiteralExpression(node, sourceFile, msg) {
    if (!typescript_1.default.isObjectLiteralExpression(node)) {
        throw new parser_1.ParserError(msg ?? `Expected an object literal, got ${typescript_1.default.SyntaxKind[node.kind]}`, {
            node,
            sourceFile,
        });
    }
...
function assertIsIdentifier(node, sourceFile, msg) {
    if (!typescript_1.default.isIdentifier(node)) {
        throw new parser_1.ParserError(msg ?? `Expected an identifier, got ${typescript_1.default.SyntaxKind[node.kind]}`, {
            node,
            sourceFile,
        });
    }
    ...
    while (properties.length > 0) {
        ...
        if (typescript_1.default.isSpreadAssignment(prop)) {
            ...
            if (!declaration.initializer || !typescript_1.default.isObjectLiteralExpression(declaration.initializer)) {
                throw new parser_1.ParserError('Expected spread object to be an object literal', {
                    sourceFile,
                    node: prop,
                });
            }
        ...
        if (!typescript_1.default.isIdentifier(prop.name) && !typescript_1.default.isStringLiteral(prop.name)) {
            throw new parser_1.ParserError('Expected property key to be an identifier or String Literal', {
                sourceFile,
                node: prop,
            });
        }
        ...
        else if (typescript_1.default.isObjectLiteralExpression(valueNode)) {
